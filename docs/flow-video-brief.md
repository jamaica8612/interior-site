...
기본 첫 화면은 실제 Flow 영상이다. 재생 버튼은 영상의 실제 재생 시계에 맞춰 스크롤한다. 모바일 스크롤 중 영상 탐색이 갱신되지 않는 사용자 보고 후, 직접 스크롤과 진행 막대는 같은 영상에서 추출한 192개 WebP 프레임을 Canvas에 그리도록 변경했다. 스크롤은 영상 메타데이터, 재생 권한, 영상 탐색에 의존하지 않는다. 현재 목표와 주변 프레임부터 최대 4개를 동시에 읽고, 이미지 보관량과 Canvas 픽셀 비율을 제한한다. 늦게 완료된 요청은 최신 목표보다 이전 프레임으로 화면을 되돌리지 않는다.
...
Node DOM 모형으로 재생, 스크롤 동기화, 정지, 탐색, 다시 보기, 동작 줄이기 설정, 메타데이터 지연, 영상 오류, 3D 라우팅을 확인했다. 추가 회귀 검사 `npm run check:film`은 영상 메타데이터가 없고 currentTime 설정이 실패하는 조건에서도 스크롤 프레임이 바뀌는지, 이미지 역순 도착, 양방향 스크롤, 요청·메모리 제한, 주소창 높이 변화와 회전 처리를 검사한다. 실제 휴대폰의 브라우저 재생은 별도 실기기 검증을 하지 않았다.
